(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/Scripts/TmpOne.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '36d64PJindNEryogkow0TG/', 'TmpOne', __filename);
// Scripts/TmpOne.js
...
cc.Class((_cc$Class = {
    ...
    onLoad: function onLoad() {
        ...
        this.node.on('touchstart', function (event) {
            ...
            if (Global.ball && Global.ball.motionNode) Global.ball.motionNode.getComponent(cc.MotionStreak).reset();
            ...
            if (Global.gameinfo.shareBallCount > 1) {
                for (var i = 0; i < Global.gameinfo.shareBallCount - 1; i++) {
                    var index = i - (Global.gameinfo.shareBallCount - 1) / 2;

                    if (index == 0) {
                        index = 1;
                    }
                    // 产生新的小球
                    _self.newBall(cc.p(Global.paddleX, Global.paddleY + 30), Global.util.calculateVelocity(Global.ball.velocity, 3.1415926 / 36 * index), Global.util.generateColorStrip3(0), true, true);
                }
            }
    ...
    initPropButtonState: function initPropButtonState() {
    ...
    // 适配iphoneX
    fitIphoneX: function fitIphoneX() {
    ...
    // 初始化对局类型
    initGameType: function initGameType() {
    ...
    // 初始化对象池
    initGameObjectPool: function initGameObjectPool() {
        // 初始化小球对象池
        this.ballPool = [];
        var initCount = 100;
        ...
        // 初始化砖块对象池
        this.brickPool = [];
        var initCountBrick = 400;
        for (var _i = 0; _i < initCountBrick; ++_i) {
            var _ball = cc.instantiate(this.brickPrefab);
            this.brickPool.push(_ball);
            this.node.addChild(_ball);
            //ball.active = false;
            _ball.setScale(0, 0);
        }
    },
        ...
        for (var i = 0; i < Global.arrProp.length; i++) {
            ...
            if (!this.isOver && prop.collision() == 2) {
                ...
                if (prop.propType == 1) {
                    ...
                    for (var _i2 = 0; _i2 < Global.platformData.maxProp2Balls; _i2++) {
                        // 产生新的小球
                        this.newBall(cc.p(this.paddle.node.position.x, this.paddle.node.position.y + Global.brickUnit), cc.p(_i2 - Global.platformData.maxProp2Balls / 2, 10), Global.util.generateColorStrip3(0), true, true);
                    }
                    ...
                    this.newBall(Global.ball.node.position, Global.util.calculateVelocity(Global.ball.velocity, -3.1415926 / 6), Global.util.generateColorStrip3(0), true, true);
                    this.newBall(Global.ball.node.position, Global.util.calculateVelocity(Global.ball.velocity, 3.1415926 / 6), Global.util.generateColorStrip3(0), true, true);
                }
            }
        }
    ...
    // 进入复活界面
    waitRevive: function waitRevive() {
    ...
    onRevive: function onRevive() {
        ...
        var _self = this;
        // 消除砖块
        var col = Global.brickCol;
        if (Global.brickRow > 3) {
            for (var i = 0; i < Global.brickRow - 3; i++) {
                for (var j = 0; j < col; j++) {
                    if (Global.arrBrick[i] && Global.arrBrick[i][j]) {
                        var row = Global.arrBrick[i][j].i;
                        var _col = Global.arrBrick[i][j].j;

                        if (Global.mapInfo[row]) Global.mapInfo[row][_col] = 0;
                        if (Global.mapBricks[row]) Global.mapBricks[row][_col] = null;
    ...
    gameOver: function gameOver() {
        ...
        for (var i = 0; i < Global.brickRow; i++) {
            for (var j = 0; j < col; j++) {
                if (Global.arrBrick[i] && Global.arrBrick[i][j]) {
                    // 产生新的小球
                    this.newBall(Global.arrBrick[i][j].position, cc.p(0, -2), Global.arrBrick[i][j].color, false, false);

                    if (Global.mapInfo[i]) Global.mapInfo[i][j] = 0;
    ...
    // 碰撞检测
    collision: function collision(dt) {
        ...
        for (var i = 0; i < Global.arrBall.length; i++) {
            ...
            if (ball.motionNode) ball.motionNode.setPosition(ball.x, ball.y);
            ...
            if (!ball.isRebound) {
                ...
                if (posY % Global.brickUnit > Global.brickUnit * 0.8 || posX % Global.brickUnit > Global.brickUnit * 0.7) {
                ...
                if (Global.mapInfo[row][col] == 1) {
                    ...
                    if (Global.ball == ball && this.isWudi) {} else {
                        ball.velocity.y *= -1;
                    ...
                    if (Global.mapBricks[row][col].getComponent("Brick").isProp) {
                        ...
                        newStar.runAction(cc.sequence(cc.moveTo(destination.mag() / 400 * 1, destination), cc.callFunc(function () {
                        ...
                        if (Global.arrBall.length < Global.platformData.maxBalls) {
                            // 产生新的小球
                            this.newBall(ball.node.position, cc.p(ball.velocity.x > 0 ? 0.5 + 2 * cc.random0To1() : -0.5 - 2 * cc.random0To1(), 0), Global.mapBricks[row][col].color, true, false);
                        }
                    }

                    Global.mapInfo[row][col] = 0;

                    var _i3 = Global.mapBricks[row][col].i;
                    var j = Global.mapBricks[row][col].j;
                    Global.arrBrick[_i3][j] = null;
    ...
    continueBricks: function continueBricks() {
        var isContinue = true;
        var col = Global.brickCol;
        for (var i = 0; i < Global.brickRow; i++) {
            for (var j = 0; j < col; j++) {
    ...
    // 游戏定时产生砖块
    updateGame: function updateGame(dt) {
    ...
    // 游戏定时产生砖块
    updateGame1: function updateGame1(dt) {
        var _this2 = this;
        ...
        if (!this.isWait && !this.isOver) {
            ...
            setTimeout(function () {
                _this2.newBricks(1);
                if (!_this2.isWait && !_this2.isOver) {
                    _this2.newBricks(1);
                }
                if (!_this2.isWait && !_this2.isOver) {
                    console.log('isWait 2', _this2.isWait);
                    setTimeout(function () {
                        _this2.newBricks(1);
                        if (!_this2.isWait && !_this2.isOver) {
                            _this2.newBricks(1);
    ...
    onPropButton1: function onPropButton1() {
        console.log('点击道具1');
        Global.main.playTouchSound();

        if (this.isBtn1) {
            for (var i = 0; i < Global.platformData.maxProp2Balls; i++) {
                // 产生新的小球
                this.newBall(cc.p(this.paddle.node.position.x, this.paddle.node.position.y + Global.brickUnit), cc.p(i - Global.platformData.maxProp2Balls / 2, 10), Global.util.generateColorStrip3(0), true, true);
            }
    ...
    onBtn1ShareSuccess: function onBtn1ShareSuccess(openGId) {
        console.log('求助成功');
    ...
    onBtn1ShareFailed: function onBtn1ShareFailed() {
        console.log('求助失败');
    ...
    onPropButton2: function onPropButton2() {
        console.log('点击道具2');
        Global.main.playTouchSound();
    ...
    onBtn2ShareSuccess: function onBtn2ShareSuccess(openGId) {
        console.log('求助成功');
    ...
    onBtn2ShareFailed: function onBtn2ShareFailed() {
        console.log('求助失败');
        console.log('1.分享取消，2.分享的不是群');
    ...
    onPropButton3: function onPropButton3() {
        var _this3 = this;
    ...
    onBtn3ShareSuccess: function onBtn3ShareSuccess(openGId) {
        console.log('求助成功');
    ...
    onBtn3ShareFailed: function onBtn3ShareFailed() {
        console.log('求助失败');
        console.log('1.分享取消，2.分享的不是群');
    ...
    // 生成地图信息
    newMapInfo: function newMapInfo(unit) {
        Global.mapInfo = [];
        Global.mapBricks = [];
    ...
    // 生成砖块
    newBricks: function newBricks(n, bricksNoNew) {
        ...
        // 生成砖块
        var generateProp = false;
        var generatePropCol = 0;
    ...
    // 生成道具
    newProp: function newProp(n, pos) {
    ...
    newBall: function newBall(pos, vel, color, isMotionStreak, isRebound) {
        var newBall = null;
    ...
    // 加入图片像素到颜色方案
    addImageToColorArray: function addImageToColorArray(imagefile) {
    ...
    // 加入颜色条带到颜色方案
    addStripToColorArray: function addStripToColorArray(stripRow) {
    ...
    // 加入颜色条带到颜色方案1
    addStripToColorArray1: function addStripToColorArray1(stripRow, stripCol, n) {
        for (var i = 0; i < stripRow; i++) {
            if (!Global.colorArray[i + Global.colorIndex]) {
                Global.colorArray[i + Global.colorIndex] = [];
            }
    ...
    // 生成颜色方案数组
    generateColorArray: function generateColorArray() {
        Global.colorArray = [];
        Global.colorIndex = 0;
    ...
    // 绘制点
    drawPoint: function drawPoint(x, y, color, size) {
    ...
    // 绘制直线
    drawLine: function drawLine(pt1, pt2, color, size) {
    ...
    // 绘制小球拖尾
    drawTail: function drawTail() {
        this.g.clear();
        for (var x in Global.arrBall) {
            // 获取小球颜色
            var colorTail = Global.arrBall[x].node.color;
            var colorTailMinValue = colorTail.getR() < colorTail.getG() ? colorTail.getR() : colorTail.getG() < colorTail.getB() ? colorTail.getG() : colorTail.getB();
            ...
            for (var i = 0; i < Global.arrBall[x].arrPoint.length; i++) {
                ...
                this.drawPoint(Global.arrBall[x].arrPoint[i].x, Global.arrBall[x].arrPoint[i].y, cc.hexToColor(colorTailHex), Global.arrBall[x].node.width / 2 - i * deltaSize);
                if (i + 1 < Global.arrBall[x].arrPoint.length) {

                    this.drawPoint((Global.arrBall[x].arrPoint[i].x + Global.arrBall[x].arrPoint[i + 1].x) / 2, (Global.arrBall[x].arrPoint[i].y + Global.arrBall[x].arrPoint[i + 1].y) / 2, cc.hexToColor(colorTailHex), Global.arrBall[x].node.width / 2 - i * deltaSize);
                }
            }
        }
    },

    initPropButtonState1: function initPropButtonState1() {
    ...
    // 适配iphoneX
    fitIphoneX1: function fitIphoneX1() {
        if (Global.fitIphoneX) {
            console.log('适配iphoneX');
            this.TopEdge.height = 120;
            this.score.node.getComponent(cc.Widget).top = 60;
    ...
    // 初始化对局类型
    initGameType1: function initGameType1() {
        var randnum = cc.random0To1() * 100;
        if (randnum >= 0 && randnum < 10) {
            this.gameType = 1;
            this.gameTypeSprite.spriteFrame = loadAtlas.getSpriteFrameFromName('icon-long');
    ...
    // 初始化对象池
    initGameObjectPool1: function initGameObjectPool1() {
        // 初始化小球对象池
        this.ballPool = [];
        var initCount = 100;
        for (var i = 0; i < initCount; ++i) {
        ...
        this.brickPool = [];
        var initCountBrick = 400;
        for (var _i4 = 0; _i4 < initCountBrick; ++_i4) {
            var _ball2 = cc.instantiate(this.brickPrefab);
            this.brickPool.push(_ball2);
            this.node.addChild(_ball2);
            //ball.active = false;
            _ball2.setScale(0, 0);
        }
    },

    initGameObjectPool2: function initGameObjectPool2() {
        var _this4 = this;
        ...
        for (var i = 0; i < Global.arrProp.length; i++) {
            ...
            if (!this.isOver && prop.collision() == 2) {
                ...
                if (prop.propType == 1) {
                    ...
                    for (var _i5 = 0; _i5 < Global.platformData.maxProp2Balls; _i5++) {
                        // 产生新的小球
                        this.newBall(cc.p(this.paddle.node.position.x, this.paddle.node.position.y + Global.brickUnit), cc.p(_i5 - Global.platformData.maxProp2Balls / 2, 10), Global.util.generateColorStrip3(0), true, true);
                    }
                } else if (prop.propType == 4) {
                    if (!this.prop4) {
                        this.prop4 = true;
    ...
    // 进入复活界面
    waitRevive1: function waitRevive1() {
        ...
        if (Global.brickRow > 3) {
            for (var i = 0; i < Global.brickRow - 3; i++) {
                for (var j = 0; j < col; j++) {
                    if (Global.arrBrick[i] && Global.arrBrick[i][j]) {
                        var row = Global.arrBrick[i][j].i;
                        var _col2 = Global.arrBrick[i][j].j;

                        if (Global.mapInfo[row]) Global.mapInfo[row][_col2] = 0;
                        if (Global.mapBricks[row]) Global.mapBricks[row][_col2] = null;
    ...
    // 碰撞检测
    collision1: function collision1(dt) {
        if (!Global.arrBall || !Global.arrBall.length) return;
        this.updateCount++;
        //if (this.updateCount % 2 == 0)  return;
        for (var i = 0; i < Global.arrBall.length; i++) {
            ...
            if (!ball.isRebound) {
                ...
                if (Global.mapInfo[row][col] == 1) {
                    ...
                    var _i6 = Global.mapBricks[row][col].i;
                    var j = Global.mapBricks[row][col].j;
                    Global.arrBrick[_i6][j] = null;
    ...
    continueBricks1: function continueBricks1() {
        var isContinue = true;
        var col = Global.brickCol;
        for (var i = 0; i < Global.brickRow; i++) {
            for (var j = 0; j < col; j++) {
...
}, _defineProperty(_cc$Class, 'updateGame1', function updateGame1(dt) {
    //console.log('tick per 5 seconds!');
    this.newBricks(1);
    ...
    //cc.director.getScheduler().schedule(this.updateGame, this, Global.brickDeltaTime -= 0.05, false);
    //}
}), _defineProperty(_cc$Class, 'updateGame12', function updateGame12(dt) {
    var _this5 = this;
    ...
    if (!this.isWait && !this.isOver) {
        ...
        setTimeout(function () {
            _this5.newBricks(1);
            if (!_this5.isWait && !_this5.isOver) {
                _this5.newBricks(1);
            }
            if (!_this5.isWait && !_this5.isOver) {
                console.log('isWait 2', _this5.isWait);
                setTimeout(function () {
                    _this5.newBricks(1);
                    if (!_this5.isWait && !_this5.isOver) {
                        _this5.newBricks(1);
...
}), _defineProperty(_cc$Class, 'onPropButton11', function onPropButton11() {
    console.log('点击道具1');
    Global.main.playTouchSound();
...
}), _defineProperty(_cc$Class, 'onBtn1ShareSuccess1', function onBtn1ShareSuccess1(openGId) {
    console.log('求助成功');
    ...
    this.propBtnBg1.active = true;
    this.propBtnBg1.runAction(cc.repeatForever(cc.sequence(cc.fadeOut(0.5), cc.fadeIn(0.5))));
}), _defineProperty(_cc$Class, 'onBtn1ShareFailed1', function onBtn1ShareFailed1() {
    console.log('求助失败');
    console.log('1.分享取消，2.分享的不是群');
    ...
    Global.main.showDialogText('分享到群才能获得获得道具');
}), _defineProperty(_cc$Class, 'onPropButton21', function onPropButton21() {
    console.log('点击道具2');
    Global.main.playTouchSound();
...
}), _defineProperty(_cc$Class, 'onBtn2ShareSuccess1', function onBtn2ShareSuccess1(openGId) {
    console.log('求助成功');
    ...
    this.propBtnBg2.active = true;
    this.propBtnBg2.runAction(cc.repeatForever(cc.sequence(cc.fadeOut(0.5), cc.fadeIn(0.5))));
}), _defineProperty(_cc$Class, 'onBtn2ShareFailed1', function onBtn2ShareFailed1() {
    console.log('求助失败');
    console.log('1.分享取消，2.分享的不是群');
    ...
    Global.main.showDialogText('分享到群才能获得获得道具');
}), _defineProperty(_cc$Class, 'onPropButton31', function onPropButton31() {
    var _this6 = this;
...
}), _defineProperty(_cc$Class, 'onBtn3ShareSuccess1', function onBtn3ShareSuccess1(openGId) {
    console.log('求助成功');
    ...
    this.propBtnBg3.active = true;
    this.propBtnBg3.runAction(cc.repeatForever(cc.sequence(cc.fadeOut(0.5), cc.fadeIn(0.5))));
}), _defineProperty(_cc$Class, 'onBtn3ShareFailed1', function onBtn3ShareFailed1() {
    console.log('求助失败');
    console.log('1.分享取消，2.分享的不是群');
    ...
    Global.main.showDialogText('分享到群才能获得获得道具');
}), _defineProperty(_cc$Class, 'newMapInfo1', function newMapInfo1(unit) {
    Global.mapInfo = [];
    Global.mapBricks = [];
...
}), _defineProperty(_cc$Class, 'newBricks1', function newBricks1(n, bricksNoNew) {
    ...
    Global.brickRow += n;
}), _defineProperty(_cc$Class, 'newProp1', function newProp1(n, pos) {
    var newProp = cc.instantiate(this.propPrefab);
    this.node.addChild(newProp);
    newProp.setContentSize(Global.brickUnit, Global.brickUnit);
    newProp.setPosition(pos);
    newProp.getComponent('Prop').setPropType(n);
    Global.arrProp.push(newProp.getComponent('Prop'));
}), _defineProperty(_cc$Class, 'newBalls1', function newBalls1() {
    for (var i = 0; i < 1; i++) {
        var newBall = cc.instantiate(this.ballPrefab);
        this.balls.addChild(newBall);
        if (Global.skinBall) {
    ...
    //Global.ball = newBall.getComponent('Ball');
}), _defineProperty(_cc$Class, 'newBall1', function newBall1(pos, vel, color, isMotionStreak, isRebound) {
    var newBall = null;
    if (this.ballPool.length > 0) {
        newBall = this.ballPool.pop();
        //console.log("从对象池创建");
    ...
    */
    ...
    //newBall.active = true;
    newBall.setScale(1, 1);
}), _defineProperty(_cc$Class, 'addImageToColorArray1', function addImageToColorArray1(imagefile) {
    //console.log("addImageToColorArray");
    // 获取图片像素数据
    var arr1 = Global.main.getImageColorArray(imagefile);
    var colorRow = 0;
...
}), _defineProperty(_cc$Class, 'addStripToColorArray11', function addStripToColorArray11(stripRow) {
    for (var i = 0; i < stripRow; i++) {
        if (!Global.colorArray[i + Global.colorIndex]) {
            Global.colorArray[i + Global.colorIndex] = [];
        }
    ...
    Global.colorIndex += stripRow;
    //console.log(Global.colorIndex);
}), _defineProperty(_cc$Class, 'addStripToColorArray11', function addStripToColorArray11(stripRow, stripCol, n) {
    for (var i = 0; i < stripRow; i++) {
        if (!Global.colorArray[i + Global.colorIndex]) {
            Global.colorArray[i + Global.colorIndex] = [];
        }
    ...
    Global.colorIndex += stripRow;
    //console.log(Global.colorIndex);
}), _defineProperty(_cc$Class, 'generateColorArray111', function generateColorArray111() {
    Global.colorArray = [];
    Global.colorIndex = 0;
    ...
    //console.log(Global.colorArray);
    Global.colorIndex = 0;
}), _defineProperty(_cc$Class, 'drawPoint1', function drawPoint1(x, y, color, size) {
    this.g.fillColor = color;
    this.g.circle(x, y, size);
    this.g.fill();
}), _defineProperty(_cc$Class, 'drawLine1', function drawLine1(pt1, pt2, color, size) {
    this.g.strokeColor = color;
    this.g.lineWidth = size;
    this.g.moveTo(pt1.x, pt1.y);
    this.g.lineTo(pt2.x, pt2.y);
}), _defineProperty(_cc$Class, 'drawTail1', function drawTail1() {
    this.g.clear();
    for (var x in Global.arrBall) {
        // 获取小球颜色
        var colorTail = Global.arrBall[x].node.color;
...
cc._RF.pop();
        ...
        //# sourceMappingURL=TmpOne.js.map
