(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/Scripts/component/bottom.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '1fd22oj3upJvJVpfuUfWnpu', 'bottom', __filename);
// Scripts/component/bottom.js
...
cc.Class({
    ...
    //创建格子:
    createAllGridNode: function createAllGridNode() {
    ...
    _selectGrid: function _selectGrid(data) {
        this.evnetIndex++;
        var gridInfo = wxGame.Global.gridInfo;
    ...
    _selectLastGrid: function _selectLastGrid() {
        //保存上次操作数据
        this.saveLastData();
    ...
    _selectWaste: function _selectWaste(selectWaste) {
    ...
    _updateGetProp: function _updateGetProp(index) {
    ...
    updateBombProp: function updateBombProp() {
        this.bombPropLabel.string = "x" + wxGame.Global.gameInfo.bombProp;
    },

    updatePowerfulProp: function updatePowerfulProp() {
    ...
    _checkOverResult: function _checkOverResult(isOver) {
    ...
    //弹出结算
    popResurtUI: function popResurtUI() {
    ...
    // 触摸区域是否在出牌区
    isTouchOnCard: function isTouchOnCard(pos) {
        ...
        if (this.girdView_2.item && this.girdView_2.item.getRootNode()) {
            var cardRect_2 = cc.rect(this.girdView_2.item.getRootNode().x + this.node.width / 2 - this.girdView_2.item.node.width / 2, this.girdView_2.item.getRootNode().y - this.girdView_2.item.getRootNode().height / 2, this.girdView_2.item.getRootNode().width, this.girdView_2.item.getRootNode().height);
        ...
        if (this.girdView_1.item && this.girdView_1.item.getRootNode()) {
            var cardRect_1 = cc.rect(this.girdView_1.item.getRootNode().x + this.node.width / 2, this.girdView_1.item.getRootNode().y - this.girdView_1.item.getRootNode().height / 2, this.girdView_1.item.getRootNode().width, this.girdView_1.item.getRootNode().height);
    ...
    //手牌不用了,归还牌池
    putCardPool: function putCardPool(card) {
    ...
    //清除手牌区
    resetHandCard: function resetHandCard() {
        if (this.girdView_1.item) {
    ...
    hideCardBag: function hideCardBag() {
    ...
    showCardBag: function showCardBag() {
        this.cardBag.stopAllActions();
    ...
    onClickCardBag: function onClickCardBag() {
    ...
    //炸弹卡牌
    onClickBombCard: function onClickBombCard() {
        this.hideCardBag();
    ...
    //万能卡牌
    onClickPowerfulCard: function onClickPowerfulCard() {
        this.hideCardBag();
    ...
    //使用交换道具
    useChangeProp: function useChangeProp(result) {
        if (result.success == 1) {
            if (this.girdView_1.item && this.girdView_2.item && result.itemId == "item:1415") {
                if (wxGame.Global.gameInfo.changeProp > 0) {
                    var _ref = [this.girdView_2.item, this.girdView_1.item];
                    this.girdView_1.item = _ref[0];
                    this.girdView_2.item = _ref[1];
    ...
    onClickRevocationBtn: function onClickRevocationBtn() {
    ...
    onUndoAdSuccess: function onUndoAdSuccess(isEnd) {
        if (isEnd) {
            wxGame.NotificationCenter.trigger(wxGame.EventType.RECOVER_LAST_OPERATION);
            wxGame.NotificationCenter.trigger(wxGame.EventType.UPDATE_REVOCATION_PROP);
        } else {
    ...
    onUndoAdFail: function onUndoAdFail() {
    ...
    //获得道具卡
    getPropCard: function getPropCard(index) {
        var that = this;
        if (this.kapaiAni) {
            this.kapaiAni.x = 0;
    ...
    //道具卡包动画
    propCardBagAni: function propCardBagAni() {
        var that = this;
        if (!that.daojubao) {
