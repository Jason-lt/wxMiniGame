...
cc.Class({
    ...
    */
    ...
    //设置倒计时
    startContinueConutDown1: function startContinueConutDown1() {
        var _this2 = this;
        ...
        this.countDownInterval1 = setInterval(function () {
            if (!_this2.pauseInterval1) {
                times--;
                if (times >= 0) {
                    //this.friendBtnLabel.string = '邀请好友' + myUtil.getSecondString(times);
                } else {
                    _this2.shareFriendState0.active = false;
                    _this2.shareFriendState1.active = true;
                    _this2.canTakeShareReward = true;
                    clearInterval(_this2.countDownInterval1);
                }
            }
        }, 1000);
    },
    ...
    createGemstone: function createGemstone() {
        ...
        this.addInterval = setInterval(function () {
            times -= 5;
            if (times >= 0) {
                var instance = cc.instantiate(_this3.gemStone);
                instance.setPosition(cc.p(0, 0));
                _this3.node.addChild(instance);
                var pos = Global.startUI.goldNode.getPosition();
                instance.getComponent('getGemStone').toPosition([pos.x, pos.y], 1);
            } else {
                clearInterval(_this3.addInterval);
            }
        }, 300);
    },
    ...
    switchBGM1: function switchBGM1() {
        Global.game.playSound('btn', 0.1);
    ...
    //创建随机掉落的钻石
    createGemstone1: function createGemstone1() {
        var _this4 = this;
