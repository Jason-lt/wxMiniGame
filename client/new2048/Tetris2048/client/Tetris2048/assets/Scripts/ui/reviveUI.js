...
cc.Class({
        ...
        if (typeof wx !== 'undefined') {
            if (Global.cdnGameConfig.totalSwith) {
                if (Global.cdnGameConfig.reviveScoreSwith) {
                    ...
                    if (canRevive) {
                        console.log('是否可以复活');
                        if (Math.random() <= Global.probabilityVideo.resurgence) {
    ...
    //看视频失败
    onWxRewardVideoFailed: function (isFail) {
        this.pauseInterval = false;
        if (isFail) {
            if(Global.cdnGameConfig.videoChangeShare) {
                this.shareGame();
            }
        }
    ...
    shareGame: function () {
        ...
        if (typeof FBInstant !== 'undefined') {
            ...
            console.log('----------------复活分享-----------------');
            ThirdAPI.shareGame('randomImg', this.onShareSuccess.bind(this), this.onShareFailed.bind(this), '123');
        } else {
            this.onShareSuccess();
        }
        ...
        /* //钻石复活
         if (Global.wxGold < Global.reviveConfig.costNum) {
             Global.game.showDialogPropText('钻石数量不足！');
             this.pauseInterval = false;
             return;
         }
    ...
    shareNotRevive: function () {
        ...
        if (typeof FBInstant !== 'undefined') {
            ...
            ThirdAPI.shareGame('randomImg', this.onShareNotReviveSuccess.bind(this), this.onShareFailed.bind(this), '123');
        } else {
