...
let ThirdAPI = {
    ...
    //存储当天是否打开登录奖励
    saveOpenLoginReward: function () {
        try {
            let storage = cc.sys.localStorage.getItem('2048-openLoginReward-data');
            let data;
            if (storage != "") {
                data = JSON.parse(storage);
            }
            ...
            let data2 = JSON.stringify(data);
            console.log('try save local revive', data2);
            cc.sys.localStorage.setItem("2048-openLoginReward-data", data2);
        } catch (error) {
            console.error(error);
        }
    },
    loadLoginReward: function () {
        try {
            let storage = cc.sys.localStorage.getItem('2048-openLoginReward-data');
            console.log('revive storage data : ' + storage);
    ...
    //清除复活次数
    clearLoginReward: function () {
        try {
            cc.sys.localStorage.removeItem('2048-openLoginReward-data');
        } catch (error) {
            console.error(error);
        }
    },
    ...
    saveGetRewardList: function (getRewardList) {
        try {
            ...
            tywx.LOGD("file = [ThirdApi] function = [saveGetRewardList] data2 + " + JSON.stringify(data2));
    ...
    loadGetRewardList: function () {
        try {
            ...
            tywx.LOGD("file = [ThirdApi] function = [loadGetRewardList] storage + " + JSON.stringify(storage));
            if (storage != "") {
                let data = JSON.parse(storage);
                if (data.data2){
                    tywx.LOGD("file = [ThirdApi] function = [loadGetRewardList] data.data2 + " + JSON.stringify(data.data2));
                    return data.data2;
                }else {
                    return [];
                }
    ...
    //
    clearGetRewardList: function () {
        try {
            cc.sys.localStorage.removeItem('2048-getRewardList-data');
        } catch (error) {
            console.error(error);
        }
    },
    ...
    //存储当天是否点击交叉导流获得奖励
    saveAdGetGuideReward: function () {
        try {
            let storage = cc.sys.localStorage.getItem('2048-adGetGuideReward-data');
            let data;
            if (storage != "") {
                data = JSON.parse(storage);
            }
            ...
            let data2 = JSON.stringify(data);
            console.log('saveAdGetGuideReward data2:', data2);
            cc.sys.localStorage.setItem("2048-adGetGuideReward-data", data2);
        } catch (error) {
            console.error(error);
        }
    },
    loadAdGetGuideReward: function () {
        try {
            let storage = cc.sys.localStorage.getItem('2048-adGetGuideReward-data');
            console.log('loadAdGetGuideReward storage : ' + storage);
            if (storage != "") {
                let data = JSON.parse(storage);
                if (data) {
                    //判断是否是当天
    ...
    //清除当天是否点击交叉导流获得奖励
    clearAdGetGuideReward: function () {
        try {
            cc.sys.localStorage.removeItem('2048-adGetGuideReward-data');
        } catch (error) {
            console.error(error);
        }
    },
