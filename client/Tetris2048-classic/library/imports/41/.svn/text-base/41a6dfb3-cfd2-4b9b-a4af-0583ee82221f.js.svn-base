...
cc.Class({
                    ...
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        try {
                            ...
                            if (ret.bgm && ret.bgm.length > 0) {
                                ...
                                for (var i = 0; i < ret.bgm.length; i++) {
    ...
    //判断是否获得复活币
    getReviveCoin: function getReviveCoin(reviveCoinState) {
    ...
    //显示排行榜界面
    showRank: function showRank(isGroup) {
        if (!Global.rankui) {
    ...
    //分享完的提示文字
    showDialogTextAlawys: function showDialogTextAlawys(text, p) {
        /*
