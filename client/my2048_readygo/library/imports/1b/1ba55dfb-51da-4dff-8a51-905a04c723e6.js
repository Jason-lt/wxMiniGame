"use strict";
cc._RF.push(module, '1ba5537UdpN/4pRkFoExyPm', 'gridController');
...
cc.Class({
    ...
    //每日限时初始化格子
    initForTimerLimit: function initForTimerLimit() {
        //根据初始时间来计算到现在多少天
        var myUtil = require('myUtil');
    ...
    checkGridMerge: function checkGridMerge() {
        var self = this;
        var downNum = 0;
        // for (var hIndex = 0; hIndex < Global.hNum; hIndex++) {
        //     for (var vIndex = 0; vIndex < Global.vNum; vIndex++) {
    ...
    closeSuccessNode: function closeSuccessNode() {
    ...
    /*是否可左右移动 */
    isMoveGrid: function isMoveGrid(hindex) {
        //横、纵坐标的索引,向下取整
    ...
    /*是否可以向下移动 */
    isMoveQuickDown: function isMoveQuickDown(isTouchDown) {
        //横、纵坐标的索引,向上取整
    ...
    //是否显示数字选择
    isShowChooseNum: function isShowChooseNum(isShow) {
        Global.game.playSound('btn', 0.1);
    ...
    //返回主界面
    goMainScene: function goMainScene() {
        Global.game.playSound('btn', 0.1);
        this.closeListen();
        this.reset();
    ...
    //可以执行炸效果
    onStartBomb: function onStartBomb() {
        //查找出需要炸掉的哪一行
        var needBombIndex = -1;
        for (var vIndex = 0; vIndex < Global.vNum; vIndex++) {
            for (var hIndex = 0; hIndex < Global.hNum; hIndex++) {
    ...
    //炸观看激励视频失败
    onBombAdFail: function onBombAdFail() {
        this.onPauseGameForOther();
