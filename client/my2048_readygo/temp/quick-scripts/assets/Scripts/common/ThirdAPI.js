...
cc._RF.push(module, '90017wfrBFDdZLEUchOUtzf', 'ThirdAPI', __filename);
...
var ThirdAPI = {
    ...
    shareGame: function shareGame(imageurl, callback, failcallback, sourcecode, channelPrefix) {
        if (Platform === 1) {
            if (callback) {
    ...
    //存储清除道具
    saveClearPropNum: function saveClearPropNum(storage) {
    ...
    getClearPropNum: function getClearPropNum() {
        try {
    ...
    //存储万能道具
    saveSuperPropNum: function saveSuperPropNum(storage) {
    ...
    getSuperPropNum: function getSuperPropNum() {
        try {
    ...
    //存储登录奖励领取状态
    saveGetRewardList: function saveGetRewardList(getRewardList) {
        try {
    ...
    loadGetRewardList: function loadGetRewardList() {
        try {
            var storage = cc.sys.localStorage.getItem('2048-getRewardList-data');
    ...
    //存储当天是否打开累积登录
    saveOpenTurnTable: function saveOpenTurnTable() {
        try {
            var storage = cc.sys.localStorage.getItem('2048-openTurnTable-data');
            var data = void 0;
            if (storage != "") {
    ...
    loadTurnTableData: function loadTurnTableData() {
        try {
            var storage = cc.sys.localStorage.getItem('2048-openTurnTable-data');
            console.log('revive storage data : ' + storage);
            if (storage != "") {
    ...
    //清除复活次数
    clearTurnTableData: function clearTurnTableData() {
        try {
    ...
    //存储累积登录天数
    saveLoginDay: function saveLoginDay(day) {
        try {
            var storage = cc.sys.localStorage.getItem('2048-loginday-data');
            var data = void 0;
            if (storage != "") {
                data = JSON.parse(storage);
            }
    ...
    loadLoginDay: function loadLoginDay() {
        try {
            var storage = cc.sys.localStorage.getItem('2048-loginday-data');
            console.log('revive storage data : ' + storage);
            if (storage != "") {
    ...
    //清除复活次数
    clearLoginDay: function clearLoginDay() {
        try {
