...
cc.Class({
    ...
    updateSinginBtnState: function updateSinginBtnState() {
    ...
    onceOpenSinginNode: function onceOpenSinginNode() {
        if (this.node.active) {
    ...
    //显示客服界面
    showServiceUI: function showServiceUI() {
        this.serviceUINode.active = true;
    },

    //关闭客服界面
    hideServiceUI: function hideServiceUI() {
    ...
    // 更新提示红点
    updateRedDot: function updateRedDot(addNum) {
    ...
    //每日挑战没有次数观看激励视频成功回调
    onDailyAdSuccess: function onDailyAdSuccess() {
        Global.gameinfo.battleTimes = Global.dailyBattleTimes;
        Global.gameinfo.battleTimes--;
        ThirdAPI.saveFriendGenStoneInfo(Global.gameinfo);
        Global.isDailyLimit = true;
    ...
    //每日挑战观看激励视频失败
    onDailyAdFail: function onDailyAdFail() {
    ...
    onMoreGame: function onMoreGame() {
        ...
        _gamesdk2.default.dataStatistics.createPFCode(function (res) {
